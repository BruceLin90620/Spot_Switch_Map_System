  ...
  current_map_id_(1)
{
    ...
    std::string config_path = this->declare_parameter<std::string>(
        "config_path", 
    ...
    // Set up publisher with QoS settings
    ...
    // Set up service for map switching
    ...
    std::shared_ptr<switch_map_interfaces::srv::SingleMap::Response> response)
{
    ...
    // Check if already using requested map
    ...
    if (pcl::io::loadPCDFile(file_names_[requested_map_id], cloud_) == -1) {
        RCLCPP_ERROR(this->get_logger(), "Failed to load PCD file: %s",
                     file_names_[requested_map_id].c_str());
        response->success = false;
    } else {
        cloud_.header.frame_id = tf_frame_;
        current_map_id_ = requested_map_id;
        RCLCPP_INFO(this->get_logger(), "Switched to PCD file: %s",
                    file_names_[requested_map_id].c_str());
        response->success = true;
    }
}
